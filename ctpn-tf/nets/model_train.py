...
def Bilstm(net, input_channel, hidden_unit_num, output_channel, scope_name):
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        lstm_out = tf.concat(lstm_out, axis=-1)#axis=-1表示按倒数第一个维度进行拼接
...
def model(image):
    ...
    rpn_conv = slim.conv2d(conv5_3, 512, 3)#在conv5上做3*3画出滑窗
    ...
    print("cls_pred_shape", cls_pred_shape.shape, tf.shape(cls_pred_shape))
    ...
    cls_prob = tf.reshape(tf.nn.softmax(tf.reshape(cls_pred_reshape, [-1, cls_pred_reshape_shape[3]])),
                          ...
                          name="cls_prob")
    print("cls_pred_reshape", cls_pred_reshape.shape, tf.shape(cls_pred_reshape))
...
def loss(bbox_pred, cls_pred, bbox, im_info):
    ...
    # transpose: (1, H, W, A x d) -> (1, H, WxA, d)
    cls_pred_shape = tf.shape(cls_pred)
    print("in loss: cls_pred_shape", cls_pred_shape)
    cls_pred_reshape = tf.reshape(cls_pred, [cls_pred_shape[0], cls_pred_shape[1], -1, 2])
    rpn_cls_score = tf.reshape(cls_pred_reshape, [-1, 2])
    rpn_label = tf.reshape(rpn_data[0], [-1])
    # ignore_label(-1)
    fg_keep = tf.equal(rpn_label, 1)
    rpn_keep = tf.where(tf.not_equal(rpn_label, -1))#fg_keep和rpn_keep有什么区别，where函数将返回其中为true的元素的索引 tf.where(tensor,a,b)  a,b为和ｔｅｎｓｏｒ相同维度的ｔｅｎｓｏｒ，将ｔｅｎｓｏｒ中的ｔｒｕｅ位置元素替换为ａ中对应位置元素，ｆａｌｓｅ的替换为ｂ中对应位置元素
    rpn_cls_score = tf.gather(rpn_cls_score, rpn_keep)#取出特定维度的数，在tensorflow中
    rpn_label = tf.gather(rpn_label, rpn_keep)
    rpn_cross_entropy_n = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=rpn_label, logits=rpn_cls_score)#传入的logits为神经网络输出层的输出，shape为[batch_size，num_classes]，传入的label为一个一维的vector，长度等于batch_size，每一个值的取值区间必须是[0，num_classes)
    ...
    regularization_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)#L2正则化
