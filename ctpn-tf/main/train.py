...
tf.app.flags.DEFINE_integer('max_steps', 500, '')#50000
tf.app.flags.DEFINE_integer('decay_steps', 30000, '')
tf.app.flags.DEFINE_float('decay_rate', 0.1, '')
tf.app.flags.DEFINE_float('moving_average_decay', 0.997, '')
tf.app.flags.DEFINE_integer('num_readers', 4, '')
tf.app.flags.DEFINE_string('gpu', '0', '')
tf.app.flags.DEFINE_string('checkpoint_path', 'checkpoints_mlt/', '')#none checkpoints_mlt/
tf.app.flags.DEFINE_string('logs_path', 'logs_mlt/', '')
tf.app.flags.DEFINE_string('pretrained_model_path', 'data/vgg_16.ckpt', '')
tf.app.flags.DEFINE_boolean('restore', False, '')#True
tf.app.flags.DEFINE_integer('save_checkpoint_steps', 100, '')#2000
...
def main(argv=None):
    ...
    # print("trainable 变量:", tf.trainable_variables())#输出说明vgg16也是在更改
    ...
    with tf.Session(config=config) as sess:
        ...
        for step in range(restore_step, FLAGS.max_steps):
            ...
            print("data[0]的形状:", np.array(data[0]).shape, "data[1]:", np.array(data[1]).shape, "Data[2]:", np.array(data[2]).shape)
